...
export function EditTaskForm({ task }: EditTaskFormProps) {
  ...
  const mutation = useMutation({
    ...
    onError: () => {
      toast({
        title: "Error",
        description: "Failed to update game event",
        variant: "destructive",
      });
    },
  });
  ...
  return (
    <>
      <DialogTitle className="text-2xl font-bold mb-4">Edit Game Event</DialogTitle>
      <Form {...form}>
        <form onSubmit={form.handleSubmit((data) => mutation.mutate(data))} className="space-y-4">
          ...
          >
            {mutation.isPending ? "Updating..." : "Update Game Event"}
          </Button>
        </form>
      </Form>
